...
## 12. MUHIM TEXNIK TUZOQ — ENTITY RESOLUTION (xabar yuborish ishlamay qolishining asosiy sababi)

GramJS (va umuman MTProto) faqat Telegram ID orqali xabar yuborishga ruxsat bermaydi — har bir kontakt uchun **`access_hash`** kerak, va bu faqat oldindan "tanishtirilgan" (resolve qilingan) kontaktlar uchun keshda mavjud bo'ladi. Agar shu qadam o'tkazib yuborilsa, natija: `Could not find the input entity for...` degan xatolik, va xabar hech qachon yuborilmaydi.

### 12.1 To'g'ri oqim

```javascript
// XATO (ko'pincha ishlamaydi):
await client.sendMessage(telegram_id, { message: text });

// TO'G'RI:
const entity = await client.getInputEntity(telegram_id); // yoki username orqali
await client.sendMessage(entity, { message: text });
```

### 12.2 Kesh to'ldirish talabi
Har bir akkaunt ulanganda (2-bo'lim, `finalize` bosqichida), backend **darhol** `client.getDialogs()` chaqirib, barcha mavjud suhbatlar/kontaktlarni keshga yuklab olishi shart. Shu orqali GramJS access_hash'larni oldindan biladi va keyinchalik istalgan kontaktga xabar yubora oladi.

### 12.3 Yangi (hali suhbat bo'lmagan) kontaktga birinchi xabar
Agar mijoz bilan hali suhbat boshlanmagan bo'lsa (masalan telefon raqamdan yangi topilgan), avval:
```javascript
const result = await client.invoke(new Api.contacts.ImportContacts({
  contacts: [new Api.InputPhoneContact({ clientId: BigInt(1), phone, firstName, lastName: "" })]
}));
// result.users[0] — endi shu userga sendMessage qilish mumkin
```

### 12.4 Event handler — har bir session uchun alohida
`TelegramConnectionManager.connect()` funksiyasi har safar yangi client ochganda, **o'sha client uchun** `addEventHandler` chaqirilishi shart (3-bo'limdagi kod namunasi) — bu global emas, har bir instance'ga alohida bog'lanadi. Agar bu unutilgan bo'lsa, faqat birinchi ulangan akkauntdan xabar kelaveradi, qolganlari kelmaydi.

---

## 13. KONTAKT QO'SHISH

Operator Chat bo'limida "+ Yangi kontakt" tugmasi orqali qo'lda mijoz qo'shishi mumkin bo'lishi kerak:

- Kiritiladi: ism, telefon raqam (yoki @username)
- Backend: agar telefon raqam bo'lsa — `contacts.ImportContacts` (12.3-bo'lim) orqali Telegram'ga tanishtiriladi; agar @username bo'lsa — `client.getEntity('@username')` orqali topiladi
- Muvaffaqiyatli topilsa — yangi `customer` yozuvi yaratiladi (agar hali mavjud bo'lmasa) va bo'sh chat ochiladi, operator darhol yozishni boshlashi mumkin
- Agar topilmasa (masalan raqam Telegram'da ro'yxatdan o'tmagan) — aniq xato ko'rsatiladi: "Bu raqam Telegram'da topilmadi"

---

## 14. QIDIRUV — @USERNAME VA BOSHQA MEZONLAR BO'YICHA

Chat bo'limi tepasida qidiruv maydoni bo'lishi kerak, quyidagilar bo'yicha qidiradi:

- **Mavjud chatlar ichida** — mijoz ismi yoki xabar matni bo'yicha (lokal DB so'rovi, tez)
- **Yangi kontakt qidirish** — agar operator `@username` formatida yozsa, tizim buni **Telegram'ning o'zidan** (`client.getEntity('@username')`) qidiradi, lokal bazada yo'q bo'lsa ham. Natija topilsa, 13-bo'limdagi kontakt qo'shish oqimi avtomatik ishga tushadi.
- Qidiruv natijalari ikki guruhga bo'lib ko'rsatiladi: "Mavjud suhbatlar" va "Telegram'da topildi (yangi)"

---

## 15. AI CODING ASSISTANT UCHUN ANIQ TOPSHIRIQ (ustuvor tartib)
...
> 13. **(Muhim tuzatish)** Xabar yuborish funksiyasini 12-bo'limga muvofiq tekshir/tuzat: `sendMessage`dan oldin albatta `getInputEntity`/`getEntity` orqali entity resolve qilinsin, har bir akkaunt ulanganda `getDialogs()` bilan kesh oldindan to'ldirilsin, va har bir client uchun `addEventHandler` alohida-alohida ulanganini tasdiqla — bu, ehtimol, hozirgi "xabar kelmayapti/yuborilmayapti" muammosining asosiy sababi
> 14. Kontakt qo'shish funksiyasini 13-bo'limga muvofiq qur (telefon/@username orqali)
> 15. Qidiruv funksiyasini 14-bo'limga muvofiq qur — lokal DB + Telegram'ning o'zidan @username qidirish
> 16. Shundan keyingina — 1-qism 6.3-6.5-bo'limlar (AI Advisor, chatbot) va 2-qism (Utel integratsiyasi) ustiga quriladi
>
